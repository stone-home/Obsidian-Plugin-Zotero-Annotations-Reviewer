...
var ObsidianService = class {
  ...
  async createLiteratureNote(metadata) {
    ...
    const path2 = (0, import_obsidian5.normalizePath)(`${this.settings.fleetingNoteFolder}/@${metadata.key} - ${fileName}.md`);
    let note;
  ...
  async getUniquePath(ann) {
    ...
    let path2 = (0, import_obsidian5.normalizePath)(`${this.settings.fleetingNoteFolder}/${base}.md`);
    let i = 1;
    while (this.app.vault.getAbstractFileByPath(path2)) {
      path2 = (0, import_obsidian5.normalizePath)(`${this.settings.fleetingNoteFolder}/${base}-${i}.md`);
      i++;
    }
    return path2;
  }
...
var AssistantView = class extends import_obsidian9.MarkdownRenderChild {
  ...
  async renderCFPContext(file, container) {
    ...
    const cfpDirPrefix = (0, import_obsidian9.normalizePath)(cfpDir + "/");
    try {
      if (!linkMatch) {
      ...
      const targetFile = resolved != null ? resolved : this.plugin.app.vault.getAbstractFileByPath(
        (0, import_obsidian9.normalizePath)(linkPath.endsWith(".md") ? linkPath : linkPath + ".md")
      );
...
function parseCcfddlYaml(text) {
  ...
  for (const entry of data) {
    ...
    if (!title) continue;
    const confs = (_c = entry.confs) != null ? _c : [];
    for (const conf of confs) {
      const year = conf.year;
      const eventAcronym = year != null ? `${title} ${year}` : title;
      const submissionDdl = deadlineFromTimeline((_d = conf.timeline) != null ? _d : []);
      items.push({
        acronym: eventAcronym,
        series: title,
        fullName: fullName || eventAcronym,
        location: ((_e = conf.place) != null ? _e : "").toString().trim() || "N/A",
        start: (_f = conf.date) == null ? void 0 : _f.toString(),
        end: void 0,
        submissionDdl,
        source: "ccfddl",
        url: (_g = conf.link) == null ? void 0 : _g.toString()
      });
    }
  }
  return items;
...
async function ensureFolder(app, path2) {
  var _a;
  const normalized = (0, import_obsidian16.normalizePath)(path2);
...
var CFPService = class {
  ...
  async fetchAll() {
    ...
    const ccfddl = ((_b = s.cfpCcfddlUrls) == null ? void 0 : _b.length) ? await fetchCcfddl(s.cfpCcfddlUrls) : [];
    const easychair = ((_c = s.cfpEasychairUrls) == null ? void 0 : _c.length) ? await fetchEasychair(s.cfpEasychairUrls) : [];
    const openresearch = ((_d = s.cfpOpenresearchUrls) == null ? void 0 : _d.length) ? await fetchOpenresearch(s.cfpOpenresearchUrls) : [];
  ...
  async syncToNotes(items) {
    ...
    const basePath = (0, import_obsidian16.normalizePath)(dir);
    ...
    for (const item of items) {
      ...
      const path2 = (0, import_obsidian16.normalizePath)(dirPath + "/" + fileName);
  ...
  async refreshCcfddlUrls() {
    ...
    try {
      const s = this.getSettings();
      const items = ((_a = s.cfpCcfddlUrls) == null ? void 0 : _a.length) ? await fetchCcfddl(s.cfpCcfddlUrls) : [];
      const synced = await this.syncToNotes(items);
  ...
  async refreshWikiCFPSeries(onProgress) {
    ...
    const basePath = (0, import_obsidian16.normalizePath)((s.cfpNoteDir || "CFP").trim() || "CFP");
    const defaultTags = Array.isArray(s.cfpDefaultTags) ? s.cfpDefaultTags : ["cfp"];
    await ensureFolder(this.app, basePath);
    const map2 = this.getSeriesMap();
  ...
  getSeriesInfoByAcronym(acronym) {
    ...
    for (const seriesName of [trimmed, pureSeriesFromAcronym(trimmed)]) {
      ...
      const seriesNotePath = (0, import_obsidian16.normalizePath)(`${dirPath}/${sanitizeFileName(seriesName)}/${sanitizeFileName(seriesName)} Series.md`);
  ...
  getCFPNoteInfoByPath(filePath) {
    var _a, _b, _c, _d;
    const dir = (this.getSettings().cfpNoteDir || "CFP").trim() || "CFP";
    const prefix = (0, import_obsidian16.normalizePath)(dir) + "/";
