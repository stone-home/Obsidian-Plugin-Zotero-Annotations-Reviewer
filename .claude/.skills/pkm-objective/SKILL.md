---
name: pkm-objective
description: PKM tier-1 — turn a discussed experiment idea into a structured Obsidian "Objective" note under docs/papers/pkm/ (flat; filename-managed). One Objective = one experiment = one paper claim. Folds in ML-systems experiment-design rigor (>=3 baselines, micro + e2e, one independent variable, controlled confounders) as the success criteria. Reads the claim registry (docs/papers/README.md + docs/papers/eval/SHL - Eval - 00 - captures.md) for claim_id/target_section; never invents claim numbers. Manual trigger only. Use when the user says "create/write an objective" after a scope has been discussed.
---
...
1. Read `docs/papers/README.md` (the claim register: `C1 — Closure (§…)`, `C2 — Self-consistency`, …) and `docs/papers/eval/SHL - Eval - 00 - captures.md` (claim → experiment-folder map).
...
4. Deduplicate: scan `docs/papers/pkm/SHL - Objective - *.md` for an existing note with the same `claim_id`. If one exists, offer to extend it instead of creating a second file.
...
- **Project abbrev:** `LWA` (Layer-wise-analysis). If any `SHL - …` note already exists in `pkm/`, reuse that abbrev — never invent a second one.
- **Filename:** `SHL - Objective - <Claim_ID> - <concise English title>.md`
  - e.g. `SHL - Objective - C1 - Lifetime Vocabulary Closure.md`
- Design companions in the same folder: `SHL - Design - <Claim_ID> - <title>.md` (link from §2).
...
Reproduce this shape exactly. Replace every `<...>` and replace `<THIS-FILE>` with the actual filename (without `.md` in wikilinks) everywhere it appears.
    ...
    **Risks / show-stoppers.** <what would fail the claim; confounders that could look like the effect>
    ...
    - [[LWMA20260202-AMIR-Memory-Prediction-v2.canvas]]
...
- The Canvas wikilink defaults to `LWMA20260202-AMIR-Memory-Prediction-v2.canvas`; the user may override or add more. Do not create or edit Canvas files from the repo.
...
3. **Deduplicate** against `docs/papers/pkm/SHL - Objective - *.md`.
